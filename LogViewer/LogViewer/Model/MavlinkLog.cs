...
namespace LogViewer.Model
{
    ...
    class MavlinkLog : IDataLog
    {
        ...
        public IEnumerable<LogEntry> GetRows(string typeName, DateTime startTime, TimeSpan duration)
        {
            foreach (var msg in GetMessages(startTime, duration))
            {
                ...
                if (typedValue != null)
                {
                    ...
                    {
                        string schemaName = GetSchemaName(typedValue);
                        if (schemaName == typeName)
                        {
                            LogEntry e = new LogEntry();
                            foreach (FieldInfo fi in typedValue.GetType().GetFields(BindingFlags.Public | BindingFlags.Instance))
                            {
                                string name = fi.Name;
                                object value = fi.GetValue(typedValue);
        ...
        private string GetSchemaName(object typedMessage)
        {
            Type t = typedMessage.GetType();
            string name = t.Name;
        ...
        private void CreateSchema(object message)
        {
            ...
            string name = GetSchemaName(message);
